...
import { Field, ModalField } from "./components";

import { useInsuranceSettingsModel } from "./hooks";
export default function InsuranceSettings() {
  const { list, open, setOpen, showAdd, setShowAdd, draft, setDraft, added, save } =
    useInsuranceSettingsModel();
  return (
    <>
      <Link
      ...
      {added && (
        <div className="card p-3 mb-4 flex items-center gap-3 bg-green-50 border-green-100 text-sm text-green-800">
          <Check className="w-4 h-4 text-green-600" /> Added <strong>{added}</strong>. New patients
          can now be assigned to this payer.
        </div>
      )}
              ...
              >
                <div className="text-left">
                  ...
                  <div className="text-xs text-slate-500 mt-0.5">
                    Compliance: ≥{ins.compliance.minHoursPerNight}h /{" "}
                    {ins.compliance.minNightsPercent}% / {ins.compliance.windowDays}d
                  </div>
                </div>
                <ChevronDown
      ...
      {showAdd && (
        ...
        >
          ...
          >
            ...
            <div className="grid grid-cols-3 gap-3">
              <ModalField
                label="Min hours/night"
                value={draft.compliance.minHoursPerNight}
                onChange={(v) =>
                  setDraft({ ...draft, compliance: { ...draft.compliance, minHoursPerNight: v } })
                }
              />
              <ModalField
                label="Min nights %"
                value={draft.compliance.minNightsPercent}
                onChange={(v) =>
                  setDraft({ ...draft, compliance: { ...draft.compliance, minNightsPercent: v } })
                }
              />
              <ModalField
                label="Window (days)"
                value={draft.compliance.windowDays}
                onChange={(v) =>
                  setDraft({ ...draft, compliance: { ...draft.compliance, windowDays: v } })
                }
              />
            </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              <ModalField
                label="Mask (days)"
                value={draft.schedule.maskDays}
                onChange={(v) =>
                  setDraft({ ...draft, schedule: { ...draft.schedule, maskDays: v } })
                }
              />
              <ModalField
                label="Tube (days)"
                value={draft.schedule.tubeDays}
                onChange={(v) =>
                  setDraft({ ...draft, schedule: { ...draft.schedule, tubeDays: v } })
                }
              />
              <ModalField
                label="Filter (days)"
                value={draft.schedule.filterDays}
                onChange={(v) =>
                  setDraft({ ...draft, schedule: { ...draft.schedule, filterDays: v } })
                }
              />
              <ModalField
                label="Pre-reminder (days)"
                value={draft.schedule.preReminderDays}
                onChange={(v) =>
                  setDraft({ ...draft, schedule: { ...draft.schedule, preReminderDays: v } })
                }
              />
              <ModalField
                label="Post-reminder (days)"
                value={draft.schedule.postReminderDays}
                onChange={(v) =>
                  setDraft({ ...draft, schedule: { ...draft.schedule, postReminderDays: v } })
                }
              />
            </div>
            <div className="mt-3 flex gap-4 text-sm">
              <label className="flex items-center gap-2 text-slate-700">
                <input
                  ...
                  onChange={(e) =>
                    setDraft({
                      ...draft,
                      schedule: { ...draft.schedule, remindersOn: e.target.checked },
                    })
                  }
                />
                Reminders on
              ...
              <label className="flex items-center gap-2 text-slate-700">
                <input
                  ...
                  onChange={(e) =>
                    setDraft({
                      ...draft,
                      schedule: { ...draft.schedule, patientEmailOn: e.target.checked },
                    })
                  }
                />
