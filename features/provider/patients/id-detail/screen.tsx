...
import { noteTypes } from "@/lib/mock-data";
import {
  ArrowLeft,
  FileBarChart,
  Plus,
...
import { Row, Dialog, DialogActions } from "./components";

import { useProviderPatientDetailModel } from "./hooks";
export default function ProviderPatientDetail() {
  ...
  return (
    <>
      ...
      {!pending && showWindow && (
        <div className="mb-5">
          <ThirtyDayWindow sessions={sessions} />
        </div>
      )}
        ...
        <div className="card p-5">
          ...
          <dl className="space-y-2 text-sm">
            ...
            <Row
              ...
              value={cm(ex?.prescribingPhysicianId)?.name ?? "—"}
            />
            <Row label="Other monitor" value={cm(ex?.otherMonitorId)?.name ?? "—"} />
          </dl>
          <p className="text-xs text-slate-500 mt-3">
            Informational tags — these do not grant data access.
          </p>
        </div>
        ...
        <div className="card p-5">
          ...
          {ex?.authorizedMonitors.length ? (
                ...
                >
                  <div>
                    <div className="text-slate-800 font-medium flex items-center gap-2">
                      {m.name}
                      <span
                        className={`badge ${m.access === "read-write" ? "badge-amber" : "badge-slate"}`}
                      >
            ...
            <p className="text-sm text-slate-500">
              No authorized monitors. Grants read-only access to an external clinician or payer.
            </p>
          )}
        </div>
        ...
        <Dialog onClose={() => setModal(null)}>
          ...
          {modal === "monitor" && (
            <>
              ...
              <p className="text-sm text-slate-500 mt-1">
                The monitor must already have a registered Authorized Monitor account. Search by
                Provider ID, name, or institution.
              </p>
              ...
              <div className="mt-3 space-y-2 max-h-64 overflow-auto">
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-slate-500 truncate">
                        {m.institution} · {m.kind} · {m.country}
                      </div>
                ...
                {monitorQuery && monitorResults.length === 0 && (
                  <p className="text-sm text-slate-500 py-2">
                    No registered monitors match. They must register an Authorized Monitor account
                    first.
                  </p>
                )}
                {!monitorQuery && (
                  <p className="text-xs text-slate-400 py-2">
                    Search the global directory of approved Authorized Monitors across all
          ...
          {modal === "deactivate" && (
            <>
              ...
              <p className="text-sm text-red-700 mt-2 bg-red-50 rounded-lg p-3">
                All device associations are lost when a patient is deactivated. The record is
                retained and can be found by enabling &quot;Inactive patients&quot;.
              </p>
