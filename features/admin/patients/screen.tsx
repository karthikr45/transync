...
import { patientFieldLabel } from "@/lib/admin-patients";

import { FieldValue } from "./components";

import { useAdminPatientsModel } from "./hooks";
export default function AdminPatients() {
  const { page, setPage, limit, setLimit, data, loading, error, list, reload } =
    useAdminPatientsModel();
  return (
    <>
      <PageHeader
        title="Patients"
      ...
      {error && (
        <div
          role="alert"
          className="card p-4 mb-4 bg-red-50 border-red-100 text-red-800 flex items-center gap-3"
        >
      ...
      <div aria-busy={loading}>
        {loading && (
          <div role="status" className="card p-8 text-center text-slate-500">
            Loading patients…
          </div>
        )}
        {!loading &&
              ...
              <table className="w-full text-sm">
                ...
                <thead className="bg-slate-50 text-xs text-slate-500">
                  <tr>
                    ...
                    {list.columns.map((key) => (
                      <th
                        key={key}
                        scope="col"
                        className="text-left font-medium px-5 py-3 whitespace-nowrap"
                      >
                        {patientFieldLabel(key)}
                      </th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {list.rows.map((patient, index) => (
                    <tr key={index} className="border-t border-slate-100 hover:bg-slate-50">
                      ...
                      {list.columns.map((key) => (
                        <td
                          key={key}
                          className="px-5 py-3 align-top min-w-[140px] max-w-md break-words"
                        >
      ...
      <nav
        ...
        className="flex items-center justify-between mt-4 gap-3 text-sm text-slate-500"
      >
        <span>
          Page {page}
