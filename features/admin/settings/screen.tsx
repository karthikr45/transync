...
import { countriesConfig } from "@/lib/mock-data";

import { useAdminSettingsModel } from "./hooks";
export default function AdminSettings() {
  const { flags, toggle } = useAdminSettingsModel();
  return (
    <>
      <PageHeader
        title="Platform settings"
        subtitle="Markets, compliance defaults and feature flags."
      />
      <MockBanner />
            ...
            {countriesConfig.map((c) => (
              <tr key={c.code} className="border-t border-slate-100">
                ...
                <td className="px-5 py-3">
                  <span
                    className={`badge ${c.status === "live" ? "badge-green" : c.status === "pilot" ? "badge-amber" : "badge-slate"}`}
                  >
                    {c.status}
                  </span>
                </td>
      ...
      <div className="card p-5">
        ...
        <p className="text-sm text-slate-500 mb-4">
          Platform-wide toggles. Some are hardware-gated.
        </p>
        <div className="space-y-3">
          {flags.map((f) => (
            <div
              key={f.id}
              className="flex items-center justify-between gap-4 border-t border-slate-100 pt-3 first:border-0 first:pt-0"
            >
              <div>
              ...
              >
                <span
                  className={`absolute top-0.5 ${f.on ? "left-5" : "left-0.5"} w-5 h-5 bg-white rounded-full transition`}
                />
              </button>
            </div>
          ))}
