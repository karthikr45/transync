...
import { Cell, DRow } from "./components";

import { useAdminApprovalsModel } from "./hooks";
    ...
    <>
      ...
      {loading ? (
        ...
        <div className="card p-8 text-center text-sm text-slate-500">
          No pending {tab === "providers" ? "providers" : "monitors"}.
        </div>
      ) : (
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  <div className="font-semibold text-slate-900">
                    ...
                    {r.userType === "home_care_provider" && r.companyName && (
                      <span className="text-slate-500 font-normal"> · {r.companyName}</span>
                    )}
                    {r.userType === "authorized_monitor" && r.institutionName && (
                      <span className="text-slate-500 font-normal"> · {r.institutionName}</span>
                    )}
                  </div>
                  <div className="text-xs text-slate-500 mt-0.5">
                    {r.email} · {r.country ?? "—"} · submitted{" "}
      ...
      {detail && (
        <div
          className="fixed inset-0 bg-slate-900/40 z-50 flex justify-end"
          onClick={() => setDetail(null)}
        >
          <div
            className="w-full max-w-md h-full bg-white shadow-xl overflow-auto"
          ...
          >
            ...
            <div className="p-5 space-y-5">
              <div>
                ...
                <span
                  className={`badge mt-1 ${detail.userType === "home_care_provider" ? "badge-amber" : "badge-slate"}`}
                >
              ...
              {detail.userType === "home_care_provider" ? (
                <div>
                  ...
                  <dl className="space-y-2 text-sm">
                    ...
                    <DRow
                      label="Address"
                      value={`${detail.address1 ?? ""} ${detail.address2 ?? ""}`.trim() || "—"}
                    />
                    <DRow
                      label="City / State"
                      value={`${detail.city ?? "—"} / ${detail.stateProvince ?? "—"}`}
                    />
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Shown to the client at login if rejected."
                />
              </div>
