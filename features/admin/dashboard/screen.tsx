...
import { useSuperAdminDashboardModel } from "./hooks";
export default function SuperAdminDashboard() {
  const { metrics, pending, activity, markets, loading, error, load } =
    useSuperAdminDashboardModel();
  return (
    <>
      <PageHeader
        title="Super Admin"
        ...
        <div className="card p-5">
          <div className="flex items-center justify-between mb-3">
            ...
            <Link
              ...
              className="text-sm text-brand-600 inline-flex items-center gap-1"
            >
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          {markets && (
            <span className="text-sm text-slate-500">
              {markets.summary.total} countries · {markets.summary.providers} providers ·{" "}
              {markets.summary.payers} payers
            </span>
          )}
        </div>
            ...
            {markets.markets.map((m) => (
              <div key={m.country} className="border border-slate-200 rounded-lg p-3">
                ...
                <div className="text-xs text-slate-500 mt-1">
                  {m.providers} provider{m.providers === 1 ? "" : "s"} · {m.payers} payer
                  {m.payers === 1 ? "" : "s"}
                </div>
              </div>
            ))}
