...
import { useMonitorPatientsModel } from "./hooks";
export default function MonitorPatients() {
  const { users, total, offset, q, setQ, loading, error, load, filtered } =
    useMonitorPatientsModel();
  return (
    <>
      <PageHeader
        title="Patients"
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan={7} className="px-5 py-8 text-center text-slate-500">
                  No users on your claimed devices.
                </td>
              </tr>
            ) : (
              filtered.map((u) => {
                ...
                return (
                  <tr key={u._id} className="border-t border-slate-100 hover:bg-slate-50">
                    ...
                    <td className="px-5 py-3">
                      <span className={`badge ${compliant ? "badge-green" : "badge-red"}`}>
                        {compliant ? "Yes" : "No"}
                      </span>
                    </td>
