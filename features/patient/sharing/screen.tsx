...
import { TYPE_LABEL, badgeFor } from "./model";

import { usePatientSharingModel, useInviteModalModel } from "./hooks";
export default function PatientSharing() {
  const { shares, setShares, loading, error, showInvite, setShowInvite, load, revoke } =
    usePatientSharingModel();
  return (
    <>
      <PageHeader
        title="Data sharing"
            ...
            {shares.length === 0 && !loading && (
              <tr>
                <td colSpan={6} className="px-5 py-8 text-center text-slate-500 text-sm">
                  No active shares yet. Invite a provider or monitor to get started.
                </td>
              </tr>
            )}
            {shares.map((s) => (
...
function InviteModal({
  ...
  const {
    ...
    submit,
    today,
  } = useInviteModalModel({ onClose, onCreated });
  return (
    <div
    ...
    >
      <form onSubmit={submit} className="card p-6 w-full max-w-md">
        ...
        <p className="text-sm text-slate-500 mt-1">
          They&apos;ll be able to view your therapy data until the date you set.
        </p>
