...
export default function MonitorPatients() {
  ...
  return (
    <>
      <PageHeader
        title="Patients"
        subtitle="Users (patients) on devices you have claimed. Read-only access; every view is logged."
        actions={
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={7} className="px-5 py-8 text-center text-slate-500">No users on your claimed devices.</td></tr>
            ) : (
              filtered.map((u) => {
                const days = u.daysUsedOver4Hours ?? 0;
                const compliant = days >= 21;
                return (
                  <tr key={u._id} className="border-t border-slate-100 hover:bg-slate-50">
                    <td className="px-5 py-3 text-slate-900 font-medium">{u.firstName ?? ""} {u.lastName ?? ""}{!u.firstName && !u.lastName ? "(unknown)" : ""}</td>
