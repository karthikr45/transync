...
async function session(page: Page, admin: boolean) {
  await page.addInitScript((admin) => {
    ...
    localStorage.setItem("tc_user", JSON.stringify({ role: admin ? "super_admin" : "user", userType: "home_care_provider", status: "approved", email: "fixture@example.test" }));
  }, admin);
}
async function api(page: Page, permissions: Permission[], rows: Record<string, WorkflowRow[]> = {}) {
  const writes: { path: string; body: Record<string, unknown>; key?: string }[] = [];
  await page.route("**/home-care/device-management/**", async (route) => {
    const req = route.request(); const url = new URL(req.url()); const path = url.pathname.split(root)[1];
    expect(req.headers().authorization).toBe("Bearer fixture-token");
    if (req.method() === "POST") {
      const body = req.postDataJSON();
      writes.push({ path, body, key: req.headers()["idempotency-key"] });
      if (path === "claim-checks") return route.fulfill({ json: { validationId: "v1", expiresAt: future(), results: body.serials.map((serial: string) => ({ serial, outcome: serial === "NEEDS-REVIEW" ? "approval_required" : "eligible", message: "Verified" })) } });
      return route.fulfill({ json: { message: "Action confirmed.", ...(path === "claims" ? { results: body.serials.map((serial: string) => ({ serial, outcome: "claimed", message: "Added to inventory" })) } : {}) } });
    }
    if (path === "context") return route.fulfill({ json: { contractVersion: 1, organizationId: "org1", permissions } });
    const items = path === "organizations" ? [{ id: "org2", name: "Approved HCP" }]
      : path === "patients" ? [{ id: "patient1", name: "Patient Example" }] : rows[path] ?? [];
    return route.fulfill({ json: { items, page: Number(url.searchParams.get("page") || 1), limit: Number(url.searchParams.get("limit") || 25), total: items.length } });
  });
  return writes;
}

test("serial batches are deduplicated without changing manufacturer case", () => {
  expect(parseSerials("Ab-1, Ab-1; ab-1\nAB-2")).toEqual({ serials: ["Ab-1", "ab-1", "AB-2"], duplicates: 1 });
...
test("HCP verification precedes claiming and never calls the legacy route", async ({ page }) => {
  await session(page, false); const writes = await api(page, ["claims:write"]);
  let legacy = false; await page.route("**/home-care/devices/upload", (route) => { legacy = true; return route.abort(); });
  ...
  expect(writes[1].key).toBeTruthy(); expect(legacy).toBe(false);
});
...
test("super admin registers a batch and allocates it to an approved HCP", async ({ page }) => {
  await session(page, true); const writes = await api(page, ["registry:write", "allocations:write"]);
  ...
  await form.getByRole("checkbox").check(); await form.getByRole("button", { name: "Register devices", exact: true }).click();
  await expect(page.getByRole("status").filter({ hasText: "Action confirmed" })).toBeVisible();
  expect(writes[0].body.devices).toEqual([{ serial: "TEST-1", model: "Example model" }, { serial: "TEST-2", model: "Example model" }]);
  ...
  await form.getByRole("checkbox").check(); await form.getByRole("button", { name: "Allocate devices", exact: true }).click();
  await expect(page.getByRole("status").filter({ hasText: "Action confirmed" })).toBeVisible();
  expect(writes[1].body.organizationId).toBe("org2");
});

test("claim review requires a reason and records the decision", async ({ page }) => {
  await session(page, true); const writes = await api(page, ["claims:review"], { "claim-requests": [{ id: "request1", serials: ["TEST-1"], status: "pending", allowedActions: ["approve", "reject"] }] });
  await page.goto("/admin/claim-requests");
  await page.getByRole("button", { name: "Approve", exact: true }).click();
  const form = page.getByRole("form", { name: "Approve request" });
  await form.getByRole("checkbox").check(); await form.getByRole("button", { name: "Approve request", exact: true }).click();
  ...
  await expect(page.getByRole("status").filter({ hasText: "Action confirmed" })).toBeVisible();
  expect(writes[0]).toMatchObject({ path: "claim-requests/request1/review", body: { decision: "approved", reason: "Order evidence checked" } });
});

test("HCP can assign an eligible patient to inventory", async ({ page }) => {
  await session(page, false); const writes = await api(page, ["assignments:write"], { inventory: [{ id: "d1", serial: "TEST-1", status: "claimed", allowedActions: ["assign"] }] });
  await page.goto("/provider/devices"); await page.getByRole("button", { name: "Assign", exact: true }).click();
  ...
  await form.getByRole("checkbox").check(); await form.getByRole("button", { name: "Assign patient", exact: true }).click();
  await expect(page.getByRole("status").filter({ hasText: "Action confirmed" })).toBeVisible();
  expect(writes[0].path).toBe("inventory/d1/assignment"); expect(writes[0].body.patientId).toBe("patient1");
});

test("transfer actions follow server allowed actions and user permissions", async ({ page }) => {
  await session(page, false); const writes = await api(page, ["transfers:write"], { transfers: [{ id: "t1", serial: "TEST-1", status: "pending_release", allowedActions: ["release"] }] });
  ...
  await form.getByRole("checkbox").check(); await form.getByRole("button", { name: "Release for transfer", exact: true }).click();
  await expect(page.getByRole("status").filter({ hasText: "Action confirmed" })).toBeVisible();
